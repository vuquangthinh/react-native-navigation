...
#import "RCCManager.h"
#import "./Helpers/RCTHelpers.h"
#import <React/RCTUIManager.h>
#import "UIViewController+Rotation.h"


NSInteger TABBAR_HEIGHT = 65;
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController {
    ...
    // ignore
    NSInteger indexOfTab = [self.viewControllers indexOfObject:viewController];
    NSDictionary* tabProp = [self.tabsProp objectAtIndex:indexOfTab];
    
    BOOL disabled = [RCTConvert BOOL:tabProp[@"props"][@"disabled"]];
    if (disabled) {
        return NO;
    }
...
- (instancetype)initWithProps:(NSDictionary *)props children:(NSArray *)children globalProps:(NSDictionary*)globalProps bridge:(RCTBridge *)bridge {
    ...
    for (NSDictionary *tabItemLayout in children) {
        ...
        BOOL withoutColor = [RCTConvert BOOL:tabItemLayout[@"props"][@"disableOverlay"]];
    ...
    // add indicator
    unsigned long divide = self.viewControllers.count;
    if([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        divide = divide + 2;
    }
    
    int indicatorSize = 3;
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width / divide, TABBAR_HEIGHT )];
    UIImageView *border = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width/divide, indicatorSize)];
    border.backgroundColor = [RCTConvert UIColor:tabsStyle[@"indicatorColor"]];
...
- (void)performAction:(NSString*)performAction actionParams:(NSDictionary*)actionParams bridge:(RCTBridge *)bridge completion:(void (^)(void))completion {
    ...
    if ([performAction isEqualToString:@"setTabBarHidden"]) {
        ...
        nextFrame.size.height = TABBAR_HEIGHT;
